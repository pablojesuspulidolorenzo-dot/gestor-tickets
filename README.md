...
- Version actual de la app: `0.1.79`
...
- v0.1.58: corrección bug "correos del hilo equivocado" — HTMX OOB swap actualiza inbox-email-list-sidebar al cambiar de hilo; nuevo partial inbox_email_list_sidebar.html; ruta /inbox/thread/{id} pasa include_email_sidebar_oob=True y threads=all_threads.
- v0.1.59: correos reales ordenados de más reciente a más antiguo; imágenes CID embebidas renderizadas vía data URI (extraídas del .eml, no requieren tabla email_attachments); sanitizador permite <style>/<html>/<head>/<body> para mayor fidelidad visual; iframe auto-redimensionable.
...
- v0.1.79: sistema completo de temas claro/oscuro: variables CSS en [data-theme="dark"], aplicado via data-theme en <html> desde sesión; nueva ruta GET/POST /settings/tema con selector visual de tarjetas; subnav de configuración ampliado con pestaña Tema.
- v0.1.78: cronología rediseñada como lista de conversación: cada correo es una fila (← entrada / → salida) con remitente, fecha y resumen IA (summary_json.nueva_informacion); acceso a summary_json por clave directa para evitar el bug de .get() en Jinja2; fallback a body_text_preview en gris si no hay IA.
- v0.1.77: cronología usa resumen IA (summary_json.nueva_informacion) en cada burbuja en lugar del body_text_preview; JOIN LATERAL sobre email_ai_processing en list_thread_messages; fallback a body_text_preview en cursiva si el correo no tiene IA procesada.
- v0.1.76: cronología del hilo rediseñada como chat: mensajes del cliente a la izquierda (fondo gris), respuestas del soporte a la derecha (fondo azul), sin asunto repetido, preview de texto, icono de adjunto, fecha y remitente compactos en cada burbuja.
- v0.1.75: script seeds 50 hilos sintéticos (scripts/seed_synthetic_data.py): 40 archivados + 10 activos/en espera, 198 emails con análisis IA, 52 síntesis de hilo, 26 tickets GLPI vinculados, EML reales con MIME multipart (HTML + texto plano, adjuntos simulados PDF/XLSX), múltiples participantes ficticios, coherencia temporal abril-junio 2026, firmas legales; fix botón ASISTENCIA que no se podía desactivar (Alpine :hx-post dinámico en lugar de hx-post estático).
- v0.1.74: sistema completo de control de hilos: auto-claim al navegar en modo operador (con OOB refresh del listado), barra de estado de control (🔒 Tienes el control / En uso por [nombre]), botón Reservar (pin persistente aunque cambies de hilo), solicitud de transferencia de control con notificación polling al titular (Ceder/Denegar), verificación server-side en Sintetizar y Reprocesar (bloquea si otro tiene el control), refresh cross-operator del listado cada 60s + instante tras claims, tabla thread_control_requests + columnas is_pinned/display_name en operator_queue_claims.
- v0.1.73: rediseño sidebar-header en dos filas ultracompactas: fila título+badge+ASISTENCIA + barra de tabs con underline-indicator estilo inline (scroll horizontal silencioso); elimina pills con bordes que ocupaban 130px → ahora ~60px.
...
- `ai_model_discovery_service.py`: `/models`, `/chat/completions`, clasificacion de errores, limpieza de `<thought>...</thought>` y extraccion de JSON de respuestas no estrictas (markdown). Funcion `_parse_llm_json` reutilizable. Constante `FALLBACK_ERROR_TYPES` para logica de fallback entre endpoints por prioridad.
